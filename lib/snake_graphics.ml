...
module Colors = struct
  ...
  let head_color = Graphics.rgb 000 000 255
...
let draw_header ~game_state consumed =
  let open Constants in
  let header_color =
  ...
  Graphics.draw_string (Printf.sprintf "Score: %d" consumed);
...
;;
(* let draw_obstacles obstacles =
  let obstacle_location = obstacles in
  draw_block obstacle_location ~color:(Colors.green)
;; *)
let rec draw_obstacles obstacles = match obstacles with
| [] -> ()
| head::body -> 
(
  let obstacle_location = head in(
  draw_block obstacle_location ~color:(Colors.green);
  draw_obstacles body)
)
;;
...
let render game =
  ...
  let obstacles = Game.obstacles game in
  let consumed = Game.consumed game in
  draw_header ~game_state consumed;
  draw_play_area ();
  draw_apple apple;
